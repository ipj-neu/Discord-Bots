...
#Base vars
border = '> ---------------------------------------------------'
pc = {'BubbleSpike': Player('Finch Valor', None, 3, 14)}
combat_list = []
initiative = []
playerready = False
dmready = False
dm_channel = 'dice-roll'
player_channel = 'dice-roll'
player_vc = 'campaign'
combat_music = ['The Elder Scrolls V - Skyrim - Combat 1', 'The Witcher 3 OST - Hunt or Be Hunted (Extended)', 'd&d Battle music', 'DOOM - HELL AWAITS US  Dark Powerful Battle Music - 1-Hour Full Mix']
...
@bot.command(name='combatmusic', help='Start combat music')
async def start_combat_music(ctx):
	await player_vc.connect()
	music = discord.utils.get(bot.voice_clients, guild=ctx.guild)
	music.play(discord.FFmpegPCMAudio(f'music_files\\{combat_music[randint(0, len(combat_music) - 1)]}.mp4'))
